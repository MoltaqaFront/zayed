  ...
  <div class="show_all_content_wrapper">
    ...
    <main>
      ...
      <div class="table_title_wrapper">
        ...
        <div class="title_route_wrapper" v-if="$can('notifications create', 'notifications')">
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.created_at`]="{ item }">
          <p class="blue-grey--text text--darken-1 fs-3" v-if="!item.created_at">-</p>
          <p v-else>{{ item.created_at }}</p>
        </template>

        <!-- Start:: Notification Content -->
        <template v-slot:[`item.data.body`]="{ item }">
          <span class="text-danger" v-if="!item.data.body"> {{ $t("TABLES.noData") }} </span>
          <div class="actions" v-else>
            <button class="btn_show" @click="showNotificationModal(item.data.body)">
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            <a-tooltip placement="bottom" v-if="$can('notifications show', 'notifications')">
              <template slot="title">
